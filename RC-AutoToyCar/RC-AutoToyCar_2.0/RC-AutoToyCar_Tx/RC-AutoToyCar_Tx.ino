...
void setup() {
  ...
  pinMode (joystickAngle_click, INPUT);

}

void loop() {

  uint8_t angle = map(analogRead (joystickAngle), 0, 1023, 105, 25) ; //가운데 90도를 기준으로 하여 줄 값 선정
  uint8_t carSpeed = map(analogRead (joystickCarSpeed), 0, 1023, 255, 0) ; //1023까지의 값을 255크기로 매핑
  uint8_t mode = digitalRead(joystickAngle_click); 
